...
def query_reversals(parent_child_pairs: list[ParentChildPair]) -> pd.DataFrame:
    parent_child_pairs_df = pd.DataFrame(columns=["child", "parent", "parent_type", "child_prediction", "gpt-4o-mini_child_logprob", "gpt-4o-mini_parent_logprob"])
    ...
    for parent_child_pair in tqdm(parent_child_pairs):
        ...
        if reverse:
            print(f"✅ {parent_child_pair.child} ← {parent_child_pair.parent} ({parent_child_pair.parent_type}) | logprob: {reverse.logprob:.9f}")
        else:
            print(f"❌ {parent_child_pair.child} ← {parent_child_pair.parent} ({parent_child_pair.parent_type}) | logprob: None")
        ...
        parent_child_pairs_df.loc[len(parent_child_pairs_df)] = {  # type: ignore
            ...
            "gpt-4o-mini_child_logprob": reverse.logprob if reverse is not None else None,
            "gpt-4o-mini_parent_logprob": parent_child_pair.logprob,
            #"gpt-4o-mini_can-find-child":
            #"gpt-4o-mini_can-find-parent":
        }  # type: ignore
...
if __name__ == "__main__":
    ...
    from src.tasks.celebrity_relations.parent_reversals import memory
    if os.path.exists(memory.location):
        print(f"🧹 Clearing Joblib cache at {memory.location}")
        memory.clear()
    ...
    # ── clear Joblib cache so get_child starts fresh ───────────────
